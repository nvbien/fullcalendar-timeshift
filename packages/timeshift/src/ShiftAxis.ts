...
import TimeshiftHeader from './TimeshiftHeader'
import TimeshiftSlats from './TimeshiftSlats'
import { TimeshiftDateProfile, buildTimeshiftDateProfile } from './timeshift-date-profile'
import TimeshiftNowIndicator from './TimeshiftNowIndicator'
import StickyScroller from './util/StickyScroller'

export interface ShiftAxisProps {
  dateProfileGenerator: DateProfileGenerator
  dateProfile: DateProfile
}

export default class ShiftAxis extends Component<ShiftAxisProps> {

  // child components
  layout: HeaderBodyLayout
  header: TimeshiftHeader
  slats: TimeshiftSlats
  nowIndicator: TimeshiftNowIndicator
  headStickyScroller: StickyScroller
  bodyStickyScroller: StickyScroller

  // internal state
  tDateProfile: TimeshiftDateProfile
  ...
  render(props: ShiftAxisProps, context: ComponentContext) {

    let tDateProfile = this.tDateProfile =
      buildTimeshiftDateProfile(props.dateProfile, context.dateEnv, context.options, props.dateProfileGenerator) // TODO: cache
  ...
  _renderSkeleton(context: ComponentContext) {
    ...
    this.header = new TimeshiftHeader(
      headerEnhancedScroller.canvas.contentEl
    )

    this.slats = new TimeshiftSlats(
      bodyEnhancedScroller.canvas.bgEl
    )

    this.nowIndicator = new TimeshiftNowIndicator(
  ...
  getNowIndicatorUnit(dateProfile: DateProfile, dateProfileGenerator: DateProfileGenerator) {
    ...
    // yuck
    let tDateProfile = this.tDateProfile =
      buildTimeshiftDateProfile(dateProfile, context.dateEnv, context.options, dateProfileGenerator) // TODO: cache
  ...
  computeDefaultSlotWidth(tDateProfile) {
    ...
    // in TimeshiftView.defaults we ensured that labelInterval is an interval of slotDuration
  ...
  dateToCoord(date) {
    let { tDateProfile } = this
    // if('additionHr' in this.context.options && this.context.options['hideHeaderText'] !== true ){
    //   let difmin = 0;
    //   if(this.context.options.timeZone !='local'){
    //     difmin = getTimezoneOffset(this.context.options.timeZone) * 60000;
    //   }
    //   let addition = createDuration({milliseconds: (parseInt(this.context.options.additionHr) + 1) * 3600000 - difmin});
    //   ['years','months','days','milliseconds','hours'].forEach((val)=>{
    //     if(!(val in addition)){
    //       addition[val] = 0;
    //     }
    //   });
    //   date = this.context.dateEnv.add(date, addition);
    //   // if(this.context.options.timeZone == 'UTC'){
