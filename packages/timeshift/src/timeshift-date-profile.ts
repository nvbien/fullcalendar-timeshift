...
export interface TimeshiftDateProfile {
  ...
  snapCnt: number
  slotCnt: number
  cellRows: TimeshiftHeaderCell[][]
}

export interface TimeshiftHeaderCell {
  text: string
  spanHtml: string
  date: DateMarker
  colspan: number
...
config.MAX_Timeshift_SLOTS = 1000
...
export function buildTimeshiftDateProfile(dateProfile: DateProfile, dateEnv: DateEnv, allOptions: any, dateProfileGenerator: DateProfileGenerator): TimeshiftDateProfile {
  let tDateProfile = {
    labelInterval: queryDurationOption(allOptions, 'slotLabelInterval'),
    slotDuration: queryDurationOption(allOptions, 'slotDuration')
  } as TimeshiftDateProfile
  ...
  /*
  console.log('label interval =', TimeshiftView.labelInterval.humanize())
  console.log('slot duration =', TimeshiftView.slotDuration.humanize())
  console.log('header formats =', TimeshiftView.headerFormats)
  console.log('isTimeScale', TimeshiftView.isTimeScale)
  console.log('largeUnit', TimeshiftView.largeUnit)
  */
...
*/
export function normalizeDate(date: DateMarker, tDateProfile: TimeshiftDateProfile, dateEnv: DateEnv): DateMarker {
  let normalDate = date
...
snaps to appropriate unit
*/
export function normalizeRange(range: DateRange, tDateProfile: TimeshiftDateProfile, dateEnv: DateEnv): DateRange {
...
export function isValidDate(date: DateMarker, tDateProfile: TimeshiftDateProfile, dateProfile: DateProfile, dateProfileGenerator: DateProfileGenerator) {
...
function validateLabelAndSlot(tDateProfile: TimeshiftDateProfile, dateProfile: DateProfile, dateEnv: DateEnv) {
  ...
  if (tDateProfile.labelInterval) {
    ...
    if (labelCnt > config.MAX_Timeshift_SLOTS) {
  ...
  if (tDateProfile.slotDuration) {
    ...
    if (slotCnt > config.MAX_Timeshift_SLOTS) {
...
function ensureLabelInterval(tDateProfile: TimeshiftDateProfile, dateProfile: DateProfile, dateEnv: DateEnv) {
  const { currentRange } = dateProfile
...
function ensureSlotDuration(tDateProfile: TimeshiftDateProfile, dateProfile: DateProfile, dateEnv: DateEnv) {
  const { currentRange } = dateProfile
...
function computeHeaderFormats(tDateProfile: TimeshiftDateProfile, dateProfile: DateProfile, dateEnv: DateEnv, allOptions: any) {
  let format1
  let format2
...
function buildIsWeekStarts(tDateProfile: TimeshiftDateProfile, dateEnv: DateEnv) {
...
function buildCellRows(tDateProfile: TimeshiftDateProfile, dateEnv: DateEnv, allOptions: any) {
    ...
    for (let row = 0; row < formats.length; row++) {
      ...
      if (isSuperRow) {
        ...
        if (
          ...
          let text = dateEnv.format(date, format);
          let duration = allOptions['duration'];
          if(!duration){
            duration = {};
          }
          if((!('days' in duration) || !isInt(duration['days']) || duration['days'] == 0)
            && (!('weeks' in duration) || !isInt(duration['weeks']) || duration['weeks'] == 0)
            && (!('months' in duration) || !isInt(duration['months']) || duration['months'] == 0)
          ){
            skip = true;
          }

          if(!skip){
            text = formatHeaderTime(date, allOptions);
          }
          console.log('date', date, text, allOptions);
          newCell = buildCellObject(date, text, rowUnits[row], allOptions, dateEnv);
        } else {
          leadingCell.colspan += 1
        }
  ...
  if(!skip){
    const lastRow = cellRows[cellRows.length - 1];
    let newRow = [];
    lastRow.forEach((val)=>{
      let text = val.text;
      let spanHtml = val.spanHtml;

        text = formatShift(val.date, allOptions);
        spanHtml = spanHtml.replace(formatHeaderTime(val.date, allOptions), text);
      
      newRow.push({...val, text, spanHtml});
    })
    if(!('resourceLabelTextSecondLine' in allOptions)){
      cellRows[cellRows.length - 1] = newRow
    }
    else{
...
function formatShift(date, options){
  const hr = date.getUTCHours();
  let duration = options['slotDurationHr'];
  if(!duration){
    duration = 0;
  }
  if(hr/duration < 1){
...
function formatHeaderTime(date, options){
  const hr = date.getHours();
  let duration = options['slotDurationHr'];
  if(!duration){
    duration = 0;
  }
  let start = hr - duration + 1;
  if(start < 0) {
    start = start + 24;
  }
  return start.toString().padStart(2,'0') + '-' + (hr+1).toString().padStart(2,'0') ;
}

function buildCellObject(date: DateMarker, text, rowUnit, allOptions: any, dateEnv: DateEnv): TimeshiftHeaderCell {
