...
import { TimeshiftDateProfile } from './timeshift-date-profile'

export interface TimeshiftHeaderProps {
  dateProfile: DateProfile
  tDateProfile: TimeshiftDateProfile
}

export default class TimeshiftHeader extends Component<TimeshiftHeaderProps> {
  ...
  render(props: TimeshiftHeaderProps) {
    this.renderDates(props.tDateProfile)
  }

  renderDates(tDateProfile: TimeshiftDateProfile) {
    ...
    findElements(
      this.tableEl.querySelectorAll('tr:first-child'), // compound selector won't work because of query-root problem
      'th .fc-cell-text'
